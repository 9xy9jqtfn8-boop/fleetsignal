...
async function checkVehicle() {
  ...
  try {
    ...
    vehicleMake =
      data.make || data.vehicleMake || data.manufacturer || null;

    vehicleColor =
      data.colour || data.color || data.vehicleColour || data.vehicleColor || null;
...
try {
  ...
  console.log("Alert system failed but ignored");
}
...
async function loadVehicles() {
  if (isLoadingVehicles) return; // 🔥 prevents duplicate calls
  ...
  for (const v of vehicles) {
    let color = "grey";
    if (v.mot_status?.includes("Expired")) color = "red";
    else if (v.mot_status?.includes("Due soon")) color = "yellow";
    else if (v.mot_status?.includes("Valid")) color = "green";
    ...
    row.innerHTML = `
      <div class="vehicle-header">
        <img src="${getVehicleImage(v.type)}" class="vehicle-icon"/>

        <div>
          <div class="vehicle-title">${v.reg}</div>
          <div class="vehicle-meta">
            ${v.make || "Vehicle"} • ${v.type || "Car"} • ${v.colour || "Unknown"}
          </div>
        </div>
      ...
      <div class="vehicle-stats">
        <span class="badge ${color}">
          ● MOT: ${v.mot_status} (${v.mot_days} days)
        </span>
