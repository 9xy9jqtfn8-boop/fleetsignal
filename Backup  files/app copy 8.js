...
let finalType = "Car"; // default

// ✅ FIRST: manual override (MOST IMPORTANT)
if (manualType && manualType.trim() !== "") {
  finalType = manualType;
} else {
  const makeLower = (vehicleMake || "").toLowerCase();

  // MOTORCYCLES (remove Suzuki!)
  const bikeBrands = ["honda", "yamaha", "kawasaki", "ducati", "triumph", "ktm", "bsa"];
  if (bikeBrands.some(b => makeLower.includes(b))) {
    finalType = "Motorcycle";
  }

  // VANS
  const vanKeywords = ["transit", "sprinter", "vivaro", "trafic", "crafter", "ducato"];
...
// MANUAL OVERRIDE (user input wins)
if (manualType) {
  finalType = manualType;
}
...
if (existingError) {
  console.error("CHECK EXISTING FAILED:", existingError);
  alert("Failed to check existing vehicle");
  return;
}
...
if (existing && existing.length > 0) {
  ...
  lastAlertSent = existing[0].last_alert_sent || null;
} else {
  // INSERT new
